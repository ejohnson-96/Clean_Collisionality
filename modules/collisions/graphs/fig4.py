...
def fig4(
    ...
    theta01 = np.loadtxt(sm.jwos(path, 'theta_i.txt'))
    theta01 = theta01[np.logical_not(np.isnan(theta01))]
    theta01n = np.loadtxt(sm.jwos(path, 'noise_theta_i.txt'))
    theta01n = theta01n[np.logical_not(np.isnan(theta01n))]
    theta10 = np.loadtxt(sm.jwos(path, 'theta_f.txt'))
    theta10 = theta10[np.logical_not(np.isnan(theta10))]
    theta10n = np.loadtxt(sm.jwos(path, 'noise_theta_f.txt'))
    theta10n = theta10n[np.logical_not(np.isnan(theta10n))]

    bn_ = bin_number
    val = value

    weg1 = np.ones_like(theta01) / float(len(theta01))
    weg2 = np.ones_like(theta01n) / float(len(theta01n))
    weg3 = np.ones_like(theta10) / float(len(theta10))
    weg4 = np.ones_like(theta10n) / float(len(theta10n))
    ...
    results, edges = np.histogram(theta01n, range=(0, 14), weights=weg2, bins=bn_, density=1, )
    binWidth = edges[1] - edges[0]
    xs, ys = make_bars(edges[:-1], results * binWidth, val)
    plt.plot(xs, ys, color='blue', linestyle='--', linewidth=1, label=r'Noise @ 0.1 ${\rm au}$', )
    ...
    results, edges = np.histogram(theta10n, range=(0, 14), weights=weg2, bins=bn_, density=1, )
    binWidth = edges[1] - edges[0]
    xs, ys = make_bars(edges[:-1], results * binWidth, val)
    plt.plot(xs, ys, color='green', linestyle='--', linewidth=1, label=r'Noise @ 1.0 ${\rm au}$', )
