...
def scalar_velocity(
    ...
    for i in range(L):
        arg_p_ = (data[p]['vp1_x'][i]) ** 2 + (data[p]['vp1_y'][i]) ** 2 + (
        data[p]['vp1_z'][i]) ** 2
        arg_a_ = (data[a]['va_x'][i]) ** 2 + (data[a]['va_y'][i]) ** 2 + (
        data[a]['va_z'][i]) ** 2
...
def scalar_temps(
    ...
    for i in range(L):
        psp_result['proton_scalar_temp_1'][i] = (
                    (2 * solar_data[p]['Tperp1'][i] + solar_data[p]['Trat1'][i]) / 3)
        psp_result['proton_scalar_temp_2'][i] = (
                    (2 * solar_data[p]['Tperp2'][i] + solar_data[p]['Trat2'][i]) / 3)
        psp_result['proton_perpar'][i] = solar_data[p]['Tperp1'][i] / \
                                         solar_data[p]['Trat1'][i]
        ...
        psp_result['alpha_scalar_temp'][i] = (2 * solar_data[a]['Ta_perp'][i] +
                                              solar_data[a]['Trat'][i]) / 3
        psp_result['alpha_perpar'][i] = solar_data[a]['Ta_perp'][i] / \
                                        solar_data[a]['Trat'][i]
